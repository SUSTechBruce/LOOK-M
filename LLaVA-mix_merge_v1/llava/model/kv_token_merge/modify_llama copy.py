...
import types
from .v433_modeling_llama import LlamaRotaryEmbedding, LlamaLinearScalingRotaryEmbedding, LlamaDynamicNTKScalingRotaryEmbedding, LlamaAttention, LlamaDecoderLayer, LlamaModel
...
class MixMerLlamaAttention_drop(H2OLlamaAttention_drop):
    def __init__(self, config: LlamaConfig):
...
class TokenMergeAttention_drop(LlamaAttention):
    # def forward(
    #     self,
    #     hidden_states: torch.Tensor,
    #     attention_mask: Optional[torch.Tensor] = None,
    ...
    #     # params setting : recent_size
    #     self.recent_size = 300
    #     self.hh_size = q_len-self.recent_size
    #     self.cache_size = self.hh_size + self.recent_size
    #     seq_len = q_len
    ...
    #     mask = torch.zeros(self.hh_score.shape, dtype=torch.bool).to(device=attn_weights.device)
    #     mask = mask.scatter(-1, keep_idx, 1)
        
    #     key_states = key_states.squeeze()[mask].view(bsz, self.num_heads, -1, self.head_dim)
    #     value_states = value_states.squeeze()[mask].view(bsz, self.num_heads, -1, self.head_dim)
    #     query_states = query_states.squeeze()[mask].view(bsz, self.num_heads, -1, self.head_dim)
    #     self.hh_score = self.hh_score[mask].view(self.num_heads, self.cache_size)

    #     attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / math.sqrt(
    #         self.head_dim
    #     )

    #     attention_mask = torch.full((self.cache_size, self.cache_size), float(-65504.), dtype=torch.float16)
    #     attention_mask = torch.triu(attention_mask, diagonal=1).to(attn_weights.device)
    ...
    #     attn_output = attn_output.transpose(1, 2).contiguous()
    #     attn_output = attn_output.reshape(bsz, self.cache_size, self.hidden_size)
    ...
    #         attn_weights = None
    #     # breakpoint()
    #     return attn_output, attn_weights, past_key_value, mask, \
...
class TokenMergeDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig):
        super().__init__(config)
        self.self_attn=TokenMergeAttention_drop(config=config)
    ...
    #         # Self Attention
    #         hidden_states, self_attn_weights, present_key_value, mask, wraptup1, wraptup2, attention_mask = self.self_attn(
    ...
    #             use_cache=use_cache,
    #         )
    #         # residual = residual.view(wraptup2).transpose(1, 2)
    #         # residual = residual.squeeze()[mask].view(*wraptup1)
...
class TokenMergeLlamaModel(LlamaModel):
    # def __init__(self, config: LlamaConfig):
    #     super().__init__(config)
    #     self.config = config
    # def forward(
    ...
    #                 use_cache=use_cache,
    #             )
    #         if idx == 0 and hidden_states.shape[1] != 1:
    #             # layer_outputs, attention_mask = layer_outputs[0], layer_outputs[1]
    #             # attention_mask = attention_mask.unsqueeze(0).unsqueeze(0)
    #             hidden_states = layer_outputs[0]
    #             # position_ids = position_ids[:, :hidden_states.shape[1]]
    #             self._common_forward()
    #         if use_cache:
    ...
    #     )
    def _common_forward(self):
        self.layers[0].forward = MethodType(LlamaDecoderLayer.forward, self.layers[0])
        self.layers[0].self_attn.forward = MethodType(LlamaAttention.forward, self.layers[0].self_attn)
    def _special_forward(self):
        self.layers[0].forward = MethodType(TokenMergeDecoderLayer.forward, self.layers[0])
        self.layers[0].self_attn.forward = MethodType(TokenMergeAttention_drop.forward, self.layers[0].self_attn)