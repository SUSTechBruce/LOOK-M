...
from .v433_modeling_llama import LlamaRotaryEmbedding, LlamaLinearScalingRotaryEmbedding, LlamaDynamicNTKScalingRotaryEmbedding
...
class LlamaAttention_drop(nn.Module):
...
class H2OLlamaAttention_drop(nn.Module):
    ...
    def forward(
        ...
        #     past_key_values = past_key_values_length,
        #     dtype=query_states.dtype,
        #     device=query_states.device,
        # )

        if attention_mask is not None:  # no matter the length, we just slice it
            causal_mask = attention_mask[:, :, :, : past_key_values_length]
            attn_weights = attn_weights + causal_mask
...
class H2OLlamaForCausalLM_drop(LlamaForCausalLM):
    def __init__(self, config):
        ...
        for layer_idx in range(num_layers):
            self.model.layers[layer_idx].self_attn = LlamaAttention_drop(config)
